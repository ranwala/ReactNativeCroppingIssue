...
#ifndef SBSDK_DOCUMENT_DETECTION_STATUS_H
#define SBSDK_DOCUMENT_DETECTION_STATUS_H

/**
 * The status of the latest document detection.
 */
// For the moment these enums are also included from within
// Obj-C and it does not support using keywords
...
typedef enum SBSDKDocumentDetectionStatus : int {
    ...
    /** A polygon was detected, but its aspect ratio should be landscape/portrait but is
       portrait/landscape. */
...
/**
 * Checks, if the status is one of the OK statuses.
