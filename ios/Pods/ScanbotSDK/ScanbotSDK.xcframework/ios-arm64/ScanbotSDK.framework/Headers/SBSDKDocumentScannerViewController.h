 ...
 */
@property (nullable, nonatomic, weak) id<SBSDKDocumentScannerViewControllerDelegate> delegate;
 ...
 */
@property (nonatomic, assign) NSTimeInterval autoSnappingDelay;
...
/** 
 * If set to NO the detection status label is always shown, if its text is non-empty.
 * Otherwise it is hidden. 
 */
@property (nonatomic, assign) BOOL suppressDetectionStatusLabel;

/** 
 * If set to NO the polygon layer is always shown, if there was a document polygon detected.
 * Otherwise it is hidden. 
 */
@property (nonatomic, assign) BOOL suppressPolygonLayer;

/** Whether the polygon-displaying layer is hidden or not. Defaults to NO. */
@property (nonatomic, assign) BOOL hidePolygonLayer;
...
/**
 * Captures the currently detected document image, as if the user would press the shutter button or the auto-snapping
 * fires. Calls the appropriate delegate functions.
 */
